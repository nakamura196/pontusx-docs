...
The Service [`consumerParameters`](/ja/docs/ddo_credential/consumer_parameters) are used to specify optional or required parameters that the consumer must provide when consuming the service. They can be used to add query strings to URLs for `GET` requests or to add additional fields into the `POST` header.
